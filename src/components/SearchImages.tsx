...
const SearchImages = ({
  ...
  const [open, setOpen] = useState(false);
  const [slides, setSlides] = useState<any[]>([]);
  const [multimedia, setMultimedia] = useState<boolean | null>(null);
  ...
  return (
    <>
      {!loading && images === null && (
        <button
          ...
          onClick={async () => {
            ...
            setMultimedia(
              typeof data.multimedia === 'boolean' ? data.multimedia : true
            );
            setLoading(false);
          }}
          className="border border-dashed border-light-200 dark:border-dark-200 hover:bg-light-200 dark:hover:bg-dark-200 active:scale-95 duration-200 transition px-4 py-2 flex flex-row items-center justify-between rounded-lg dark:text-white text-sm w-full"
      ...
      {images !== null && multimedia === false && (
        <div className="text-center text-gray-500 py-4">
          Images are only available for healthcare or drug discovery queries.
        </div>
      )}
      {images !== null && images.length > 0 && multimedia !== false && (
        <>
          <div className="grid grid-cols-2 gap-2">
            {images.length > 4
              ? images.slice(0, 3).map((image, i) => (
